...
int nivelNo(no **raiz, int proc){
  if ((*raiz)==NULL || proc == (*raiz)->info){
    return 0;
  }
  
  //Ando
  if (proc < (*raiz)->info){
    return 1+ nivelNo(&(*raiz)->esq, proc);
  }else{
    return 1+ nivelNo(&(*raiz)->dir, proc);
  }
}
//////////////Nivel de um nó da arvore
...
//altura da arvore
int altura(no **raiz){
  //Para arvores sem raiz, retorna -1!
  if ((*raiz)==NULL){return -1;}
  
  int esq = 1+altura(&(*raiz)->esq);
  int dir = 1+altura(&(*raiz)->dir);
  
  if (esq > dir){return esq;}else{
    return dir;
  }
